...
export function Home() {
    ...
    const teste = [{
      key: '0',
      label: 'PrimeReact',
      data: 'https://www.primefaces.org/primereact/',
      icon: 'pi pi-fw pi-inbox',
      children: [
          {
              key: '0-0',
              label: 'GitHub',
              data: 'https://github.com/primefaces/primereact',
              icon: 'pi pi-fw pi-cog',
              children: [
                  { key: '0-0-0', label: 'rerer', icon: 'pi pi-fw pi-file', data: 'https://primereact.org/autocomplete/' },
                  { key: '0-0-1', label: 'sdsds', icon: 'pi pi-fw pi-file', data: 'https://primereact.org/installation/' },
                  { key: '0-0-2', label: 'sdsgfa', icon: 'pi pi-fw pi-file', data: 'https://primereact.org/autocomplete/' },
                  { key: '0-0-3', label: '0-0-3', icon: 'pi pi-fw pi-file', data: 'https://primereact.org/installation/' },
                  { key: '0-0-4', label: '0-0-4', icon: 'pi pi-fw pi-file', data: 'https://primereact.org/autocomplete/' },
                  { key: '0-0-5', label: '0-0-5', icon: 'pi pi-fw pi-file', data: 'https://primereact.org/installation/' }
              ]
          },
          {
              key: '0-1',
              label: 'Forum',
              data: 'https://primereact.org/theming/',
              icon: 'pi pi-fw pi-home',
              children: [{ key: '0-1-0', label: 'Getting Started', icon: 'pi pi-fw pi-file', data: 'https://blocks.primereact.org/' }]
          },
          {
              key: '0-2',
              label: 'Changelog',
              data: 'https://www.primefaces.org/primereact/changelog',
              icon: 'pi pi-fw pi-home',
              children: [{ key: '0-2-0', label: 'v7.0.0', icon: 'pi pi-fw pi-file', data: 'https://blocks.primereact.org/free' }]
          }
      ]
  }]
    ...
    const endContent = (
        <React.Fragment>
            <div className="flex align-items-center gap-2">
                <Avatar image="https://github.com/caiorodr.png" shape="circle" />
                <span className="font-bold text-bluegray-50">Caio Rodrigues</span>
            </div>
        </React.Fragment>
    );
    ...
    function handleTabClose(e: any) {
        console.log(e)
        const newTabs = tabs.filter((_tab: any, tabIndex: number) => tabIndex !== e.index);
        setTabs(newTabs);
    ...
    const handleToggle = () => {
        setIsExpanded(prevState => !prevState); // Inverte o estado de isExpanded
        //setVisible(prevState => !prevState); // Inverte o estado de isOpen
      };

    return (    
        <S.Layout isOpen={visible} isExpanded={isExpanded}>
          <S.Header>
            <Toolbar start={startContent} center={centerContent} end={endContent} />
          </S.Header>
  
          <S.Aside isOpen={visible} isExpanded={isExpanded}>
            <div className="min-h-screen ">
              <div id="app-sidebar-2" className="surface-section h-screen hidden lg:block flex-shrink-0 absolute lg:static left-0 top-0 z-1 border-right-1 surface-border select-none" >
                  <div className="flex flex-column h-full">
                      <div className="flex align-items-center justify-content-between px-4 pt-3 flex-shrink-0 overflow-hidden gap-8	">
                          ...
                          <span>
                              <Button type="button" onClick={handleToggle}  rounded outlined size={"small"}></Button>
                          </span>
                      </div>
                      <div >
                          <ul className="list-none p-3 m-0">
                              <li>
                                  ...
                                  <ul className="list-none p-0 m-0 overflow-hidden">
                                      <li>
                                          <a className="p-ripple flex align-items-center cursor-pointer p-3 border-round text-700 hover:surface-100 transition-duration-150 transition-colors w-full gap-3	">
                                      ...
                                      <li>
                                          <StyleClass nodeRef={btnRef2} selector="@next" enterClassName="hidden" enterActiveClassName="slidedown" leaveToClassName="hidden" leaveActiveClassName="slideup">
                                              <a ref={btnRef2} className="p-ripple flex align-items-center cursor-pointer p-3 border-round text-700 hover:surface-100 transition-duration-150 transition-colors w-full gap-3	">
                                          ...
                                          <ul className="list-none py-0 pl-3 pr-0 m-0 hidden overflow-hidden transition-all transition-duration-400 transition-ease-in-out">
                                              <li>
                                                  <StyleClass nodeRef={btnRef3} selector="@next" enterClassName="hidden" enterActiveClassName="slidedown" leaveToClassName="hidden" leaveActiveClassName="slideup">
                                                      <a ref={btnRef3} className="p-ripple flex align-items-center cursor-pointer p-3 border-round text-700 hover:surface-100 transition-duration-150 transition-colors w-full gap-3	">
                                                          <i className="pi pi-chart-line mr-2"></i>
                                                  ...
                                                  <ul className="list-none py-0 pl-3 pr-0 m-0 hidden overflow-hidden transition-all transition-duration-400 transition-ease-in-out">
                                                      <li>
                                                          <a className="p-ripple flex align-items-center cursor-pointer p-3 border-round text-700 hover:surface-100 transition-duration-150 transition-colors w-full gap-3	gap-3	">
                      ...
                      <div className="mt-auto overflow-hidden">
                          ...
                          <a v-ripple className="m-3 flex align-items-center cursor-pointer p-3 gap-2 border-round text-700 hover:surface-100 transition-duration-150 transition-colors p-ripple gap-3">
          ...
          <S.Content>
            ...
            >
              {tabs.map((tab) => (
                <TabPanel key={tab.id} header={tab.title} closable>
                  {tab.content}
                </TabPanel>
              ))}
            </S.TabViewCustom>
